...
MStatus CurveIKCmd::doIt(const MArgList& args)
{
	...
	MVector startToEndEff = m_localJointsPos[m_localJointsPos.size()-1] - m_localJointsPos.front();
	...
	MFnTransform curJoint(leafJointDagPath);
	curJoint.setTranslation(effectorCurvePos, MSpace::kWorld);
	return MS::kSuccess;
}
